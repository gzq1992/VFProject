...
void HumanModel::ReadMatlabFile(const char* filename,HumanPart& part)
{
	...
	else
	{
		part.Colors.resize(M, N);
		part.Colors.setOnes();
		part.Colors = 0.75 * part.Colors;
	}

	mxArray* pFace = matGetVariable(pmatFile, "Face");
...
#if 1
void HumanModel::ReadObjFile(const char* filename, HumanPart& part, size_t NumberOfVertices, size_t NumberOfFaces)
{
...
void HumanModel::SaveModel2Obj(const char* filename, Eigen::MatrixXd& Vertices, Eigen::MatrixXi& Faces)
{
	std::ofstream out;
	out.open(filename);
...
{
	NowBoundary.ReadBoundaryWeightFile("MatlabFile\\BoundaryWeight.mat");
