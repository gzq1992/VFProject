...
Boundary::Boundary(const char* filename) : NumberOfBoundaryVertex(0), VertexAndNeighborsInHead(NULL), VertexAndNeighborsInBody(NULL)
{
...
void Boundary::ReadMatlabFile(const char* filename)
{
...
void Boundary::ReadBoundaryWeightFile(const char* filename)
{
	MATFile* pmatFile = matOpen(filename, "r");
	if (pmatFile == NULL)
	{
